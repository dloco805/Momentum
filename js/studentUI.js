...
const StudentUI = (() => {
    ...
    function collectChangesSinceLastMeeting(student) {
        ...
        student.journey.followUps.forEach((item) => {
            ...
            if (item.status === "completed" || item.completedAt) {
                add("success", "Follow-up completed", item.title || "Next step");
            } else {
                add("followup", "Follow-up added", item.title || "Next step",
    ...
    function renderLastTimeWeMet(student) {
        ...
        return `<section class="last-time-panel"><div class="panel-header"><div><p class="eyebrow">Continuity</p><h3>Last Time We Met</h3></div><span>${escapeHtml(DateUtils.formatDateTime(latest.meetingDate, latest.meetingTime))}</span></div>
            ...
            <article><strong>Open next steps</strong>${followups.length ? `<ul>${followups.map((item)=>`<li>${escapeHtml(item.title || "Next step")}</li>`).join("")}</ul>` : `<p>No open next steps.</p>`}</article></div></section>`;
    }
                    ...
                    <details class="today-open-items">
                        ...
                        <div class="today-open-items-list">
                            ${followUps.slice(0, 5).map((item) => `
                                <article>
                                    <span>Next Step</span>
                                    <strong>${escapeHtml(item.title || "Next step")}</strong>
                                    <p>${item.dueDate
            ...
            <article class="binder-work-card binder-tone-${escapeHtml(tone)}
                ...
                ${item ? `
                    ...
                    <div class="binder-work-card-actions">
                        ...
                        <button class="button button-secondary button-small"
                            type="button"
                            data-action="add-next-step"
                            data-student-id="${escapeHtml(studentId)}"
                            data-linked-type="${isProject ? "project" : "internship"}"
                            data-linked-item-id="${escapeHtml(item.id)}"
                            data-linked-label="${escapeHtml(
                                item.title || item.organization || title
                            )}">
                            + Add Next Step
                        </button>
            ...
            <div class="minimal-momentum-page">
                <section class="minimal-momentum-grid">
                        ...
                        ${connections.length ? `
                            <div class="minimal-connection-list">
                                ${connections.slice(0, 4).map(({partner, engagement}) => `
                                    <div class="minimal-connection-item">
                                        <span>
                                        ...
                                        <button class="button button-secondary button-tiny"
                                            ...
                                            data-action="add-next-step"
                                            data-student-id="${escapeHtml(student.id)}"
                                            data-linked-type="community"
                                            data-linked-item-id="${escapeHtml(engagement.id)}"
                                            data-linked-label="${escapeHtml(partner.organization)}">
                                            + Next Step
                                        </button>
                                    </div>
                                `).join("")}
                    ...
                    <article class="binder-work-card binder-tone-next-step
                        ...
                        ${nextSteps.length ? `
                            <div class="minimal-next-step-list">
                                ${nextSteps.slice(0, 4).map((item) => `
                                    <div class="minimal-next-step-item">
                                        <strong>${escapeHtml(
                                            item.title || item.description || "Next step"
                                        )}</strong>
                                        <span class="next-step-link-badge">
                                            ${escapeHtml(nextStepLinkLabel(student, item))}
                                        </span>
                                        <small>${escapeHtml(
                        ...
                        ` : `
                            <p class="empty-copy">
                                Add a general step or connect it to a project,
                                internship, or community connection.
                            </p>
                        `}
                        <button class="button button-primary button-small" type="button"
                            data-action="add-next-step"
                            data-student-id="${escapeHtml(student.id)}"
                            data-linked-type="general">
                            + Add Next Step
                        </button>
                    </article>
...
<div class="minimal-site-line">
    ...
    function nextStepLinkLabel(student, item) {
        const type = String(item.linkedType || "general").toLowerCase();
        if (item.linkedLabel) {
            const prefix = {
                project: "Project",
                internship: "Internship",
            ...
            return type === "general" ? "General" : `${prefix} · ${item.linkedLabel}`;
        }

        if (type === "project") {
            const project = student.journey.currentProjects.find((entry) =>
                entry.id === (item.linkedItemId || item.linkedProjectId)
            );
            return project ? `Project · ${project.title}` : "Project";
        }

        if (type === "internship") {
            const internship = student.journey.internships.find((entry) =>
                entry.id === (item.linkedItemId || item.linkedInternshipId)
            );
            return internship
                ? `Internship · ${internship.organization || internship.title}`
                : "Internship";
        }

        if (type === "community") {
            const connection = studentCommunityConnections(student).find(({engagement}) =>
                engagement.id === item.linkedItemId
            );
            return connection
                ? `Community · ${connection.partner.organization}`
    ...
    function nextStepReferenceOptions(student, selectedValue = "general|") {
        const projects = student.journey.currentProjects.filter((item) =>
            item.status !== "completed" && item.status !== "archived" && !item.archived
        );
        const internships = student.journey.internships.filter((item) =>
            item.status !== "completed" && item.status !== "archived" && !item.archived
        );
        const connections = studentCommunityConnections(student);
        ...
        return `
            ${option("general|", "General — student-level next step")}
            ${projects.length ? `
                <optgroup label="Projects">
                    ${projects.map((item) => option(
                        `project|${item.id}`,
            ...
            ${internships.length ? `
                <optgroup label="Internships">
                    ${internships.map((item) => option(
                        `internship|${item.id}`,
                        item.organization || item.title || "Untitled Internship"
                    )).join("")}
                </optgroup>
            ` : ""}
            ${connections.length ? `
                <optgroup label="Community Connections">
                    ${connections.map(({engagement, partner}) => option(
    ...
    function nextStepFormTemplate(studentId, preset = {}) {
        const student = StudentManager.getStudent(studentId);
        if (!student) return "";

        const selectedValue = preset.linkedType && preset.linkedType !== "general"
            ? `${preset.linkedType}|${preset.linkedItemId || ""}`
            : "general|";
        ...
        return `
            <div class="modal-backdrop" data-modal-backdrop>
                <section class="modal modal-small" role="dialog" aria-modal="true">
                    <div class="modal-header">
                        <div>
                            <p class="eyebrow">${escapeHtml(displayName(student))}</p>
                            <h2>Add Next Step</h2>
                        </div>
                        <button class="icon-button" type="button"
                            data-action="close-modal" aria-label="Close">×</button>
                    </div>
                    <form id="studentNextStepForm">
                        <input type="hidden" name="studentId"
                            value="${escapeHtml(student.id)}">
                        <div class="modal-body">
                            <div class="form-field">
                                <label for="studentNextStepTitle">Next Step *</label>
                                <input id="studentNextStepTitle" name="title" required
                                    placeholder="What needs to happen next?">
                            </div>
                            <div class="form-field">
                                <label for="studentNextStepReference">Linked To</label>
                                <select id="studentNextStepReference" name="linkReference">
                                    ${nextStepReferenceOptions(student, selectedValue)}
                                </select>
                            </div>
                            <div class="form-field">
                                <label for="studentNextStepDueDate">Due Date</label>
                                <input id="studentNextStepDueDate" name="dueDate" type="date">
                            </div>
                            <div class="form-field">
                                <label for="studentNextStepNotes">Notes</label>
                                <textarea id="studentNextStepNotes" name="description"
                                    placeholder="Optional context or details"></textarea>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button class="button button-secondary" type="button"
    ...
    function openNextStepForm(studentId, preset = {}) {
        if (!StudentManager.getStudent(studentId)) return;
        state.modalRoot.innerHTML = nextStepFormTemplate(studentId, preset);
        document.body.style.overflow = "hidden";
        window.setTimeout(() => {
            state.modalRoot.querySelector("[name='title']")?.focus();
        }, 0);
    }
    ...
    function handleClick(event) {
        ...
        if (action === "add-next-step") {
            openNextStepForm(studentId, {
                linkedType: actionTarget.dataset.linkedType || "general",
                linkedItemId: actionTarget.dataset.linkedItemId || "",
                linkedLabel: actionTarget.dataset.linkedLabel || ""
            });
        } else if (action === "view-journey-item") {
            openJourneyView(
                studentId,
            ...
            if (collection === "followUps") {
                ...
                App.showToast("Next step marked complete.");
                renderDetail(studentId);
            }
    ...
    function handleSubmit(event) {
        if (event.target.id === "studentNextStepForm") {
            ...
            const [linkedTypeRaw, linkedItemId = ""] = String(
                data.get("linkReference") || "general|"
            ).split("|");
            const linkedType = linkedTypeRaw || "general";
            let linkedLabel = "";

            if (linkedType === "project") {
                linkedLabel = student.journey.currentProjects.find((item) =>
                    item.id === linkedItemId
                )?.title || "Project";
            } else if (linkedType === "internship") {
                const item = student.journey.internships.find((entry) =>
                    entry.id === linkedItemId
                );
                linkedLabel = item?.organization || item?.title || "Internship";
            } else if (linkedType === "community") {
                linkedLabel = studentCommunityConnections(student).find(({engagement}) =>
                    engagement.id === linkedItemId
                )?.partner.organization || "Community Connection";
            }
            ...
            StudentManager.addJourneyItem(studentId, "followUps", {
                ...
                linkedItemId,
                linkedLabel,
                linkedProjectId: linkedType === "project" ? linkedItemId : "",
                linkedInternshipId: linkedType === "internship" ? linkedItemId : ""
            });

            closeModal();
            state.activeProfileTab = "myMomentum";
            renderDetail(studentId);
            App.showToast("Next step added.");
            return;
        }
