...
const StudentUI = (() => {
    ...
    function renderAdvisorWorkspace(student) {
        ...
        const latest = studentViewLatestMeeting(student);
        const todaysQuestion = typeof QuestionLabUI !== "undefined"
            ? QuestionLabUI.getStudentQuestion(student)
            : "";

        return `
            <div class="today-minimal-view">
                ${todaysQuestion ? `
                    <section class="student-today-question">
                        <span>Today’s Question</span>
                        <blockquote data-student-today-question>
                            ${escapeHtml(todaysQuestion)}
                        </blockquote>
                        <button class="button button-secondary button-small"
                            type="button"
                            data-action="new-student-today-question"
                            data-student-id="${escapeHtml(student.id)}">
                            New Question
                        </button>
                    </section>
    ...
    function renderBinderCover(student) {
        ...
        const openItems = binderOpenItems(student);
        const goals = activeBinderGoals(student);
        const projectGoals = goalsForProject(student, project);
        const internshipGoals = goalsForInternship(student, internship);
        const generalGoals = goals.filter((goal) =>
            !goal.linkedProjectId && !goal.linkedInternshipId
        );
        const recent = binderRecentActivity(student, 4);
        const latest = studentViewLatestMeeting(student);
        const transportation = student.profile.transportation;
        const transportationText = transportation.modes?.length
            ? transportation.modes.join(" + ")
            : transportation.primaryMode ||
                (transportation.hasReliableAccess ? "Reliable access" : "Not recorded");

        return `
            <div class="binder-cover">
                <section class="binder-cover-meta binder-cover-meta-without-mood">
                    <div class="binder-cover-links">
                        ${student.profile.portfolioUrl
                            ? portfolioLink(student, {
                                label: "Google Site",
                                className: "binder-text-link"
                            })
                            : `<span>Google Site not added</span>`
                        }
                        <span>Transportation: ${escapeHtml(transportationText)}</span>
                        <span>${openItems.length} to follow up</span>
                        <span>Last meeting: ${latest
                            ? escapeHtml(DateUtils.formatDateTime(
                                latest.meetingDate,
                                latest.meetingTime
                            ))
                ...
                <section class="binder-section binder-priority-section">
                    <div class="binder-work-grid binder-priority-grid">
                        <article class="binder-work-card binder-tone-focus">
                            <span>Current Focus</span>
                            <h3>${escapeHtml(
                                student.profile.currentFocus ||
                                "No current focus selected"
                            )}</h3>
                            <p>${escapeHtml(
                                student.profile.focusWhy ||
                                "Add what matters most for this student right now."
                            )}</p>
                            <div>
                                <small>Next Action</small>
                                <strong>${escapeHtml(
                                    student.profile.focusNextAction ||
                                    generalGoals[0]?.nextSteps?.[0] ||
                                    "No next action recorded"
                                )}</strong>
                            </div>
                            <button type="button"
                                data-action="edit-student-focus"
                                data-student-id="${escapeHtml(student.id)}">
                                Edit Focus
                            </button>
                        </article>

                        <article class="binder-work-card binder-tone-goal binder-current-goals-card ${goals.length ? "" : "binder-work-missing"}">
                            ...
                            ${goals.length ? `
                                <div class="binder-current-goal-list">
                            ...
                            ` : `
                                <p>Add the priorities the student is actively working toward.</p>
                            `}
                            <button type="button"
                                data-action="add-journey-item"
                                data-student-id="${escapeHtml(student.id)}"
                ...
                <section class="binder-section binder-work-section">
                    <div class="binder-section-heading">
                        <h3>Current Work</h3>
                    </div>
                    <div class="binder-work-grid">
                        ${binderWorkCard(
                            "Current Project",
                            project,
                            "project",
                ...
                <section class="binder-section">
                    ...
                    ` : `<p class="empty-copy">Nothing waiting for next step.</p>`}
                </section>
    ...
    function renderBinderProjects(student) {
        ...
        return `
            <div class="binder-work-page binder-projects-page">
                <section class="binder-section project-page-hero
                    ${activeProjects.length ? "" : "binder-work-missing"}">
                    <div class="binder-section-heading">
                        <div>
                            <p class="eyebrow">Student work and passion</p>
                            <h3>Projects</h3>
                        </div>
                        <button class="button button-primary button-small" type="button"
                            data-action="add-journey-item"
                        ...
                        <div class="project-featured-list">
                            ${activeProjects.map((project) => `
                                <article class="project-featured-card">
                                    <div class="project-featured-heading">
                                        <div>
                                            <span>${escapeHtml(
                                                project.phase ||
                                                formatJourneyStatus(project)
                                            )}</span>
                                    ...
                                    <p>${escapeHtml(
                                        project.projectQuestion ||
                                        project.description ||
                                        "No project question or description recorded yet."
                                    )}</p>

                                    <div class="project-featured-details">
                                        <div>
                                    ...
                                    <section class="project-inline-timeline">
                                        <div class="panel-header">
                                            <h4>Recent Updates</h4>
                                        </div>
                                        ${renderActivityLog({
                                            ...project,
                                            activityLog: sortedActivityLog(project).slice(0, 4)
                                        })}
                                    </section>
                                </article>
                            `).join("")}
                    ...
                    ` : `
                        <div class="empty-state compact-empty-state">
                            ...
                            <p>
                                Add the project this student is passionate about,
                                then use Quick Update to build its story over time.
                            </p>
                        </div>
                    `}
                </section>

                <section class="binder-section project-portfolio-link">
                    <div class="binder-section-heading">
                        <div>
                            <p class="eyebrow">Published work</p>
                            <h3>Google Site Portfolio</h3>
                        </div>
                    </div>
                    ...
                    ` : `
                        <div class="portfolio-link-row is-missing">
                            <div>
                                <strong>No Google Site linked</strong>
                                <p>Add the student's published Google Site portfolio address.</p>
                            </div>
                            <button class="button button-primary button-small"
                                type="button"
                                data-action="edit-portfolio-link"
                                data-student-id="${escapeHtml(student.id)}">
                                Add Google Site Link
                            </button>
                        </div>
                    `}
                </section>

                ${projects.some((item) => item.status === "completed" || item.completedAt) ? `
                    <section class="binder-section">
                        <div class="binder-section-heading">
                            <h3>Completed Projects</h3>
                        </div>
    ...
    function renderActiveInternshipWorkspace(student, internship) {
        if (!internship) {
            return `
                <article class="internship-workspace-card is-empty">
                    <span>Current Internship</span>
                    <h3>No active internship</h3>
                    <p>Add an internship when the student begins a placement or work experience.</p>
                    <button type="button"
                        data-action="add-journey-item"
                        data-student-id="${escapeHtml(student.id)}"
                        data-collection="internships">
        ...
        const transportationText = transportation.modes?.length
            ? transportation.modes.join(", ")
            : transportation.primaryMode || "Not recorded";

        const details = [
            ["Employer", internship.organization || internship.title || "Not recorded"],
            ["Position", internship.title || "Not recorded"],
            ["Status", formatJourneyStatus(internship)],
            ["Schedule", internship.schedule || "Not recorded"],
            ["Hours", internship.hoursPerWeek || "Not recorded"],
            ["Supervisor", internship.supervisor || "Not recorded"],
            ["Dates", [
                internship.startDate ? formatDate(internship.startDate) : "",
        ...
        return `
            <article class="internship-workspace-card">
                <div class="internship-workspace-heading">
                    <div>
                        <span>Current Internship</span>
                        <h3>${escapeHtml(
                            internship.organization ||
                        ...
                        ${internship.organization && internship.title
                            ? `<p>${escapeHtml(internship.title)}</p>`
                            : ""
                        }
                    </div>
                    <span class="badge badge-success">${escapeHtml(
                        formatJourneyStatus(internship)
                    )}</span>
                </div>

                <dl class="internship-workspace-details">
                    ${details.map(([label, value]) => `
                        <div>
                            <dt>${escapeHtml(label)}</dt>
                            <dd>${escapeHtml(value)}</dd>
                ...
                <div class="internship-workspace-focus">
                    <div>
                ...
                <div class="internship-workspace-actions">
                    <button class="button button-primary button-small" type="button"
                        data-action="edit-viewed-journey-item"
                        data-student-id="${escapeHtml(student.id)}"
                        data-collection="internships"
    ...
    function renderBinderInternships(student, existing) {
        const internship = binderCurrentInternship(student);

        return `
            <div class="binder-work-page binder-internships-page">
                <section class="binder-section binder-current-internship-section">
                    <div class="binder-section-heading">
                        <h3>Current Internship</h3>
                    </div>
                    <div class="binder-single-work-card">
                        ${renderActiveInternshipWorkspace(student, internship)}
                    </div>
                </section>

                <section class="binder-section binder-linked-goals">
                    <div class="binder-section-heading">
                    ...
                    ${renderGoalRows(
                        activeBinderGoals(student).filter((goal) =>
                            goal.linkedInternshipId ||
                            (!goal.linkedProjectId && goal.category === "Internship")
                        ),
                        student.id,
                        "No active internship goals."
                    )}
                </section>

                <section class="binder-section binder-opportunity-pipeline-section">
                    ${existing.career}
                </section>
                ...
                <div class="detail-actions minimal-detail-actions">
                    ...
                    <details class="student-action-menu">
                        ...
                        <div class="student-action-menu-list">
                            ...
                            <button type="button" data-action="add-observation"
                                data-student-id="${escapeHtml(student.id)}">Observation</button>
    ...
    function focusFormTemplate(student) {
        return `
            <div class="modal-backdrop" data-modal-backdrop>
                <section class="modal focus-modal" role="dialog" aria-modal="true"
                    aria-labelledby="focusFormTitle">
                    <div class="modal-header">
                        <div>
                            <p class="eyebrow">Student priority</p>
                            <h2 id="focusFormTitle">Edit Focus</h2>
                        </div>
                        <button class="icon-button" type="button"
                            data-action="close-modal" aria-label="Close">×</button>
                    </div>
                    ...
                    <form id="studentFocusForm">
                        ...
                        <div class="modal-body">
                            <div class="form-field">
                                ...
                                <input name="currentFocus"
                                    value="${escapeHtml(student.profile.currentFocus)}"
                                    placeholder="What matters most right now?">
                            </div>

                            <div class="form-field">
                                <label>Why this matters</label>
                                <textarea name="focusWhy"
                                    placeholder="Why is this the current priority?">${escapeHtml(student.profile.focusWhy)}</textarea>
                            </div>

                            <div class="form-field">
                                <label>Next action</label>
                                <textarea name="focusNextAction"
                                    placeholder="What is the next concrete step?">${escapeHtml(student.profile.focusNextAction)}</textarea>
                            </div>
                        </div>
                ...
                <section class="modal modal-small" role="dialog" aria-modal="true"
                    ...
                    <div class="modal-header">
                        <div>
                            ...
                            <p>${collection === "currentProjects"
                                ? "Capture what happened, evidence, and the next step."
                                : "Add progress without changing the main record."
                            }</p>
                        </div>
                        <button class="icon-button" type="button" data-action="close-modal" aria-label="Close">×</button>
                    </div>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-grid">
                                ...
                                ${collection === "currentProjects" ? `
                                    <div class="form-field full-width">
                                        <label for="recordUpdateEvidence">Evidence</label>
                                        <textarea id="recordUpdateEvidence" name="evidence"
                                            placeholder="Photo, document, link, product, feedback, or other evidence"></textarea>
                                    </div>
                                ` : ""}
                                <div class="form-field full-width">
    ...
    function handleClick(event) {
        ...
        if (action === "view-journey-item") {
            ...
            document.body.style.overflow = "hidden";
        } else if (action === "edit-student-focus") {
            const student = StudentManager.getStudent(studentId);
            if (!student) return;
            state.modalRoot.innerHTML = focusFormTemplate(student);
            document.body.style.overflow = "hidden";
            ...
            document.body.style.overflow = "hidden";
        } else if (action === "new-student-today-question") {
            const output = document.querySelector(
                "[data-student-today-question]"
            );
            const questions = [
                ...QuestionLabUI.questionsForSituation("no-interests"),
                ...QuestionLabUI.questionsForSituation("project"),
                ...QuestionLabUI.questionsForSituation("future"),
                ...QuestionLabUI.questionsForSituation("confidence")
            ];
            if (output && questions.length) {
                const current = output.textContent.trim();
                const available = questions.filter((item) => item !== current);
    ...
    function handleSubmit(event) {
        ...
        if (event.target.id === "studentFocusForm") {
            event.preventDefault();
            const data = new FormData(event.target);
            const studentId = String(data.get("studentId") || "");
            StudentManager.updateStudent(studentId, {
                profile: {
                    ...
                    focusWhy: data.get("focusWhy"),
                    focusNextAction: data.get("focusNextAction")
                }
            });
            closeModal();
            renderDetail(studentId);
            App.showToast("Student focus updated.");
            return;
        }
