...
const FollowUpCenter = (() => {
        ...
        return `
            <article class="${className}">
                <div class="followup-center-main">
                    <h4>${escapeHtml(item.title || "Next step")}</h4>
                    <p>${escapeHtml(item.description || "No details added.")}</p>
                </div>

                <div class="followup-center-meta">
                    <strong>${escapeHtml(item.studentName)}</strong>
                    <p>${escapeHtml(item.linkedLabel || "Student next step")}</p>
                </div>
        ...
        if (!items.length) {
            state.content.innerHTML = `
                <div class="empty-state">
                    ...
                    <p>Adjust the filters or create a next step for a student.</p>
            ...
            <div class="modal-backdrop">
                <section class="modal modal-small" role="dialog" aria-modal="true" aria-labelledby="globalFollowUpTitle">
                    <div class="modal-header">
                        <div>
                            <h2 id="globalFollowUpTitle">${followUp ? "Edit Next Step" : "New Next Step"}</h2>
                            <p>A next step is a clear action that should happen after a conversation or plan.</p>
                        </div>
                    ...
                    <form id="globalFollowUpForm">
                        <div class="modal-body">
                            ...
                            <div class="form-field">
                                <label for="globalFollowUpName">Next Step *</label>
                        ...
                        <div class="modal-footer">
                            ...
                            <button class="button button-primary" type="submit">${followUp ? "Save Changes" : "Create Next Step"}</button>
                        </div>
                    </form>
                </section>
            </div>
    ...
    function handleClick(event) {
        ...
        if (action === "new-global-followup") {
            ...
            App.showToast("Next step completed.");
        } else if (action === "reopen-global-followup") {
            StudentManager.updateJourneyItem(studentId, "followUps", followUpId, {
                status: "open",
                completedAt: ""
            });
            App.showToast("Next step reopened.");
    ...
    function handleSubmit(event) {
        ...
        if (followUpId) {
            ...
            App.showToast("Next step updated.");
        } else {
            StudentManager.addJourneyItem(studentId, "followUps", payload);
            App.showToast("Next step created.");
        }
