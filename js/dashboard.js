...
const Dashboard = (() => {
    ...
    function itemRow(item) {
        const actionAttributes = item.plannerId
            ? `data-dashboard-planner-id="${esc(item.plannerId)}"`
            : item.studentId
                ? `data-action="view-student" data-student-id="${esc(item.studentId)}"`
                : "";
    ...
    function initialize(container) {
        ...
        root?.addEventListener("click", (event) => {
            const target = event.target.closest("[data-dashboard-planner-id]");
            if (!target) return;
            const plannerId = target.dataset.dashboardPlannerId || "";
            App.navigate("calendar");
            if (!CalendarUI.openPlannerEvent(plannerId)) {
                App.showToast("That calendar item could not be opened.", "error");
            }
        });
        [
