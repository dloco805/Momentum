...
const CheckInCenter = (() => {
    ...
    function renderStudentCheckInCard(row) {
        ...
        const historyItems = row.items.slice(1);
        ...
        return `
            <article class="checkin-student-card checkin-tone-${tone}">
                    ...
                    <div class="card-actions">
                        ...
                        ${historyItems.length ? `
                            <button class="button button-secondary button-small"
                                ...
                                data-student-id="${escapeHtml(row.studentId)}"
                                aria-expanded="false"
                                aria-controls="checkinHistory-${escapeHtml(row.studentId)}">
                                View History
                            </button>
                        ` : ""}
                    </div>
                ...
                ${historyItems.length ? `
                    <div id="checkinHistory-${escapeHtml(row.studentId)}"
                        class="checkin-history-panel"
                        data-checkin-history="${escapeHtml(row.studentId)}"
                        hidden>
                        ${historyItems.map(renderCheckIn).join("")}
                    </div>
                ` : ""}
            </article>
        `;
    ...
    function handleClick(event) {
        ...
        if (target.dataset.action === "new-checkin") {
            ...
            const studentId = String(target.dataset.studentId || "");
            const panel = [...document.querySelectorAll("[data-checkin-history]")]
                .find((item) => item.dataset.checkinHistory === studentId);
            if (panel) {
                const opening = panel.hidden;
                panel.hidden = !opening;
                target.textContent = opening ? "Hide History" : "View History";
